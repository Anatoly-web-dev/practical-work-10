...
let fruitsFiltered = [], minValue, maxValue, count = 0;
...
function filterFruits() {
	...
	fruitsFiltered = fruits.filter((item) =>
		item.weight >= minValue && item.weight <= maxValue);
...
function mixFruits() {
	...
	while (fruits.length > 0) { // пока текущий массив не опустеет
	...
	if (compareArrays(fruits, newFruits) && fruits.length > 0) { // массивы равны
...
function comparationColor(a, b) {
	const priority = ['красный', 'оранжевый', 'коричневый', 'желтый', 'зеленый',
	...
	bubbleSort(items, comparation) {
		...
		for (let i = 0; i < items.length - 1; i++) {
			...
			for (let j = 0; j < items.length - 1 - i; j++) {
				if (comparation(items[j], items[j + 1])) { // сравниваем элементы
...
sortChangeBtn.addEventListener('click', () => {
	sortKind = sortKindInput.textContent === 'quickSort' ? 'bubbleSort' : 'quickSort';
...
function getValues() {
	...
	if (addedKind.length > 0) {
		addedKind = addedKind[0].toUpperCase() + addedKind.slice(1).toLowerCase().trim();
	}
...
// проверяем все ли поля ввода заполнены, если нет показываем предупреждение
function addNewFruits() {
	if (addedKind !== '' && addedColor !== '' && addedWeight !== '') {
		let newFruit = {}; // пустой объект
		newFruit.kind = addedKind;
		newFruit.color = addedColor;
		newFruit.weight = addedWeight;
		fruits.push(newFruit);
		display(fruits)
	} else {
		popupAddFruit.classList.add('popup-add-fruit-open'); // показываем предупреждение 
		wrapper.style.zIndex = -1; // пользователь не взаимодейтсвует с осн. контентом
	}
}
...
addFruitBtn.addEventListener('click', () => {
	getValues();
	addNewFruits();
	// очищаем инпуты после добавления фрукта, если это удобно
	//kindInput.value = colorInput.value = weightInput.value = ''; 
});

// закрываем pop-up 2
document.addEventListener('click', (e) => {
	if (e.target.closest('.popup-add-fruit__btn') ||
...
function setBorderColor() {
	// css классы
	let fruitsClassList = ['fruit_violet', 'fruit_green', 'fruit_carmazin', 'fruit_yellow',
		'fruit_lightbrown', 'fruit_orange', 'fruit_cyan', 'fruit_blue', 'fruit_pink'];
